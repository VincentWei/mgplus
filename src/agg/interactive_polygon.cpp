...
namespace agg
{
    ...
    bool interactive_polygon::on_mouse_button_down(double x, double y)
    {
        ...
        m_node = -1;
        m_edge = -1;
        for (i = 0; i < m_num_points; i++)//click on point
        {
            if(sqrt( (x-xn(i)) * (x-xn(i)) + (y-yn(i)) * (y-yn(i)) ) < m_point_radius)
            {
                ...
                m_node = int(i);
                ret = true;
                //printf("click down on vertex: m_dx=%f, m_dy=%f, m_node=%d\n", m_dx, m_dy, m_node);
                break;
            }
        }
        ...
        if(!ret)
        {
            for (i = 0; i < m_num_points; i++)
            {
                if(check_edge(i, x, y))
                {
                    ...
                    m_edge = int(i);
                    ret = true;
                    //printf("click down on edge: m_dx=%f, m_dy=%f, m_edge=%d\n", m_dx, m_dy, m_edge);
                    break;
                }
            }
        }
        ...
        if(!ret)
        {
            if(point_in_polygon(x, y))
            {
                ...
                //printf("click down in polygon: m_dx=%f, m_dy=%f, m_node=%f\n", m_dx, m_dy, m_node);
                ret = true;
            }
        }
        //printf("pylygon.cpp mouse click down: m_node=%d, m_edge=%d, m_dx=%d, m_dy=%d, x=%d, y=%d\n", 
        //                    m_node, m_edge, m_dx, m_dy, x, y);
        return ret;
    }
    ...
    bool interactive_polygon::on_mouse_move(double x, double y)
    {
        ...
        double dx;
        double dy;
        //printf("pylygon.cpp mouse move:m_node=%d, m_edge=%d, m_dx=%d, m_dy=%d, x=%d, y=%d\n", 
        //                    m_node, m_edge, m_dx, m_dy, x, y);
